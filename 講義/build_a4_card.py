...
rich([('起手・45字・照抄　',True,RED),('本案依都市計畫法主要計畫（§15、§32）之指導、細部計畫（§22）之土地使用管制，並依都市設計審議原則管制量體與開放空間，→〔接本年議題〕',False,None)],after=0.5)
rich([('4個不套　',True,None),('純理論programming→不起手，只結尾橋接｜題目已括出法規面→豁免，直接破題｜都更vs危老→另一法系，不套§15/§32｜公設立體化→改套「多目標使用辦法」',False,None)],after=0.5)
rich([('收尾　',True,None),('全案於建築技術規則（停車／無障礙／防火避難／綠建築）基本盤上，導入屋頂光電與出流管制減洪，落實安全、永續而具公共性之實質環境。→「以下就設計說明之」',False,None)],after=0.5)
...
rich([('法源鏈　',True,None),('主計定位置｜細計定數字｜都審定形態｜技則定底線｜敷地＝把上面畫成一張圖',False,None)],after=0.2)
para('└ 都審無都計法明文授權→實體依據＝細計之都設基準／土管要點；程序依據＝地方自治法規',indent=0.15,after=0.2)
para('└ 細計 §22 七款含「地下室開挖之限制」＝圖上標開挖率的法源出身',indent=0.15,after=0.2)
para('└「敷地計畫」非法定名詞→法定用語＝建築基地（建築法 §11）',indent=0.15,after=1.2)

# ---------- 右欄（續排）：設計期 ----------
head('設計 70 分｜0:50–3:55')
rich([('0:50 抄圖說清單→畫完打✓　',True,RED),('用詞照抄題目不改字｜缺1張＝硬扣，小張也有分，空白＝0',False,None)],after=0.3)
para('□全區配置　□空間/機能組織（水平＋垂直）　□剖立面（要幾向畫幾向）　□外部空間局部透視（要幾處畫幾處）←114漏此張　□一樓平面（量體格不能取代）　□地下開挖範圍　□指北針・比例尺 S=1/___・圖名',indent=0.15,after=0.5)
...
para('不合→改方塊，絕不改②｜題目給「坪」先×3.3｜法定停車空間免計容積（技則）｜1車位≈40㎡、停車樓地板≈容積 ¼~⅓',indent=0.15,after=0.5)
...
rich([('剖面・缺一即扣　',True,RED),('畫大：長≥配置圖寬、高≥8cm；剖線切過開放空間＋主量體',False,None)],after=0.2)
...
rich([('透視・缺＝硬扣　',True,RED),('開卷即鉛筆框 15×10cm，末30分填滿。醜不扣、缺必扣',False,None)],after=0.2)
...
rich([('水綠4數字　',True,None),('滯洪 ≥基地面積×0.045 m³/㎡（<300㎡免）｜保水 >0.5×法定空地比率｜綠化用喬木複層非草皮｜出流管制 ≥1ha・都計區新建 ≥0.2ha',False,None)],after=0.5)
...
para('算式寫圖旁：開挖面積÷基地面積＝○% ≤70% …ok!　虛線須避開：退縮帶／老樹樹冠根系／遺構水圳／鄰地界線',indent=0.15,after=0.2)
para('旁註 覆土≥1.0m（供喬木）・防水高程 FL＋○.○○　法源出身＝細計 §22「地下室開挖之限制」',indent=0.15,after=0.5)
